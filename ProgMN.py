...
def GrafiBisec():
	global expresion1
	expresion1 = (EntradaFu.get())
	pv = float(EntradaPV.get())
	sv = float(EntradaSV.get())
	#x = numpy.linspace(0,10,100)
	#x = range(int(pv),int(sv))
	x = numpy.arange(float(pv),float(sv),0.01)
	plt.plot(x,[Bis(i) for i in x])
	#plt.plot(x,Bis(x))
	plt.xlabel('Eje X')
	plt.ylabel('Eje Y')
	plt.grid()
...
def BiseccionRes():
	global expresion1
	expresion1 = (EntradaFu.get())
	xa = float(EntradaPV.get())
	xb = float(EntradaSV.get())
	xc = ((xa  + xb)/ 2)
	err1 = float(EntradaError.get())

	while abs(Bis(xc)) > err1:
		xc = ((xa  + xb)/ 2)
		if(Bis(xa)*Bis(xc)) < 0:
			xb = xc
		else:
			xa = xc 
		xc = round(xc,6)
	label3_R.grid(row=6,column=0,columnspan=2,pady=10)
	label3_R.configure(text= " Resultado Final: {0:.6f}".format(round(xc,6)))
...
primerframe.configure(background='firebrick', width= 1050, height =560)
titulo = Label(primerframe,text="Método de bisección",bg="firebrick",fg="white", font=("Berlin Sans FB", 25))
titulo.grid(row=0,column=0,pady=10,columnspan =2)
ColoFu = Label(primerframe,text="Coloque funcion en terminos de x: ",bg="firebrick",fg="white", font=("Berlin Sans FB", 15))
ColoFu.grid(row=1,column=0,pady=10)
PriVa= Label(primerframe,text="Introduce el valor de inicio(x1): ",bg="firebrick",fg="white", font=("Berlin Sans FB", 15))
PriVa.grid(row=2,column=0,pady=10)
SeguVa= Label(primerframe,text="Introduce el valor para finalizar(x2): ",bg="firebrick",fg="white", font=("Berlin Sans FB", 15))
SeguVa.grid(row=3,column=0,pady=10)
labelerror= Label(primerframe,text="Introduce el valor del error: ",bg="firebrick",fg="white", font=("Berlin Sans FB", 15))
labelerror.grid(row=4,column=0,pady=10)
EntradaFu = Entry(primerframe, justify= CENTER, font=("Berlin Sans FB", 15),width=20)
EntradaFu.grid(row=1,column=1)
EntradaPV = Entry(primerframe, justify= CENTER, font=("Berlin Sans FB", 15),width=20)
EntradaPV.grid(row=2,column=1)
EntradaSV = Entry(primerframe, justify= CENTER, font=("Berlin Sans FB", 15),width=20)
EntradaSV.grid(row=3,column=1)
EntradaError = Entry(primerframe, justify= CENTER, font=("Berlin Sans FB", 15),width=20)
EntradaError.grid(row=4,column=1)
LeBotone = Button(primerframe, bg='white',text= "Como poner una funcion",command = ventaanayuda, font=("Berlin Sans FB", 15))
LeBotone.grid(row=5,column=0,pady=20)
LeBotoneCalculador = Button(primerframe, bg='white',text= "Calcular",command = BiseccionRes, font=("Berlin Sans FB", 15))
LeBotoneCalculador.grid(row=5,column=1,pady=20)
BotonGraficador=Button(primerframe, bg='white',text= "Grafica de la funcion",command = GrafiBisec, font=("Berlin Sans FB", 15))
BotonGraficador.grid(row=5,column=2,pady=20)

label3_R = Label(primerframe,bg="firebrick",fg="white", font=("Berlin Sans FB", 25))
